...
def build_dataset(config, query_dataset, input_min_text_length=2, input_max_text_length=8):
    ...
    ds = ds.filter(lambda x: len(x["question"]) > 33, batched=False)
    ...
    def tokenize(sample):
        ...
        s = 'Question: If a lantern is not for sale, where is it likely to be? Choices - antique shop, house, dark place\n\n' \
                                 'Reasoning: A house is the only place that is not likely to sell things\n\n' \
                                 'Answer: house\n\n'
        sample["input_ids"] = tokenizer.encode(s)[: input_size()]
        sample["query"] = tokenizer.decode(sample["input_ids"])
        return sample
        #
        # sample["query"] = 'Question: If a lantern is not for sale, where is it likely to be? Choices - antique shop, house, dark place\n\n' \
        #                          'Reasoning: A house is the only place that is not likely to sell things\n\n' \
        #                          'Answer: house\n\n' \
